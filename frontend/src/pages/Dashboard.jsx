...
cursor: pointer;
&:hover {
  background-color:rgb(47, 178, 180);
}
`
const Dashboard = (CUId) => {
  ...
  const deleteExam = (id) => {
    axios.delete(`http://localhost:3000/exam/${id}`).then((response) => {
  ...
  return (
    <>
      ...
      <Container>
        <Wrapper>
          <Popup
            trigger={<CreateButton >Design Game </CreateButton>}
            modal
            nested
          >
            {close => (
              <div style={{ fontSize: "12px", backgroundColor: "#393E46", width: "400px" }}>
                ...
                <form onSubmit={handleName}>
                  <div style={{ width: "100", borderBottom: "1px solid gray", fontSize: "18px", padding: "5px", color: "white" }}>New Game</div>
                  <div style={{ width: "100%", padding: "10px 5px" }}>
                    <input type="text" style={{ width: "90%", padding: "5px", borderRadius: "6px", border: "none" }} placeholder='Enter title for your game' onChange={e => setExamName(e.target.value)} required /><br />
                  </div>
